...
namespace BookMenu
{
    ...
    public sealed partial class BookSearch : Page
    {
        ...
        public async void Search(int s)
        {
            tt.Text = xx[s][4];
            if (xx[s][4] == "0")
            {
                xx[s][4] = "1";
                var dialog = new MessageDialog("借閱成功", "借閱");
            ...
            {
                var dialog = new MessageDialog("此書已被借閱", "借閱");
            //   await dialog.ShowAsync();
                dialog.Commands.Add(new UICommand("是", YesCommand));
         dialog.DefaultCommandIndex = 0;
         await dialog.ShowAsync();
     ...
     void YesCommand(IUICommand command)
        ...
        public async void conbime(string x)   //分割AND判斷
        {
         ...
         xx = new string[num][];
           ...
           bool boo = false;
                  ...
                  {
                      var dialog = new MessageDialog("查無此作者或書名", "查詢");
                      //   await dialog.ShowAsync();
                      dialog.Commands.Add(new UICommand("是", YesCommand));
                      dialog.DefaultCommandIndex = 0;
                      await dialog.ShowAsync();
                ...
                private async void Button_Click_1(object sender, RoutedEventArgs e)
        {
           ...
           StorageFolder storageFolder = KnownFolders.PicturesLibrary;
           StorageFile storageFile = await storageFolder.GetFileAsync("aa.txt");
           await FileIO.WriteTextAsync(storageFile, ttd);
        }
