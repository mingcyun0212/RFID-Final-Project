...
namespace BookMenu.Common
{
    /// <summary>
    /// NavigationManager 協助在頁面間進行巡覽。 它提供的命令用來
    /// 向後及向前巡覽，以及註冊標準的滑鼠及鍵盤。
    /// 用來返回及向前之快速鍵。 此外它還整合了 SuspensionManger
    /// 在巡覽時用以處理其間之處理程序生命週期管理和狀態管理
    /// 頁面。
    /// </summary>
    /// <example>
    /// 如果要使用 NavigationManager，請依循這兩個步驟或
    /// 以 BasicPage 或非 BlankPage 的任何其他頁面項目範本開頭。
    /// 
    /// 1) 在某處建立 NaivgationHelper 的執行個體，例如
    ...
    /// </code>
    /// 
    /// 2) 註冊頁面以便在頁面參與時呼叫 NavigationManager
